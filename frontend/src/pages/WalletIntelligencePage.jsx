...
import api from '../api';
import { collection, errorMessage, formatAmount } from '../contracts';
...
export default function WalletIntelligencePage() {
  ...
  const inspectWallet = async (nextAddress = address, nextChain = chain) => {
    ...
    try {
      ...
      const all = collection(txResponse.data, 'transactions');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="card" style={{ padding: '16px 20px' }}>
        <form onSubmit={event => { event.preventDefault(); inspectWallet(); }} style={{ display: 'flex', gap: 12 }}>
          <input className="form-input mono" value={address} onChange={e => setAddress(e.target.value)} placeholder="Wallet address" style={{ flex: 1 }} />
      ...
      {transfers.length > 0 && <div className="panel" style={{ margin: 0 }}><div className="panel-header"><h3>Validated Transfers ({transfers.length})</h3></div><div style={{ overflowX: 'auto' }}>
        <table className="data-table"><thead><tr><th>Hash</th><th>Direction</th><th>Counterparty</th><th>Amount</th><th>Event time</th><th>Finality</th></tr></thead><tbody>
          {transfers.map(tx => { const outgoing = tx.source_address === address; return <tr key={tx.id}><td className="mono">{tx.tx_hash}</td><td>{outgoing ? 'Outgoing' : 'Incoming'}</td><td className="mono">{outgoing ? tx.target_address : tx.source_address}</td><td className="mono">{formatAmount(tx.amount, tx.asset)}</td><td>{tx.event_time || 'Unknown'}</td><td>{tx.finality_state || 'Unknown'}</td></tr>; })}
        </tbody></table></div></div>}
      {risk && <div className="intel-panel"><div className="intel-panel-header"><h4>Case Risk Context</h4></div><div className="intel-panel-body"><div className="risk-gauge"><div className="gauge-value">{risk.score ?? '—'}</div><div className="gauge-label">{risk.tier || 'unknown'} · {risk.risk_kind}</div></div><p style={{ color: 'var(--text-secondary)', marginTop: 12 }}>{risk.assessment_state === 'assessed' ? 'Deterministic case-level decision support; not a wallet-owner verdict or ML probability.' : 'Risk has not been assessed for the selected trace.'}</p></div></div>}
