...
def evaluate(run: AnalysisRun, db: Session) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    score = min(100, sum(item["contribution"] for item in assessed))
    result = {"risk_kind": "deterministic_composite", "score": score, "tier": risk_tier(score),
              "policy_version": POLICY_VERSION, "assessment_state": "assessed" if assessed else "unknown",
              "findings": findings, "ml_prediction": None,
              "limitations": ["Rule output is decision support, not a criminality determination."]}
